        ...
        if isinstance(streams, list):
            for stream in streams:
                if isinstance(stream, dict) and stream.get("type") == "latlng":
                    ...
                    if lats and lngs:
                       # ⚡ Bolt Optimization: Use itertools.islice with a generator expression to lazily evaluate
                       # and skip elements BEFORE performing the expensive round() and list allocation operations.
                       return [[round(lat, 5), round(lng, 5)] for lat, lng in itertools.islice(((lat, lng) for lat, lng in zip(lats, lngs) if lat is not None and lng is not None), 0, None, 4)]
    return None
