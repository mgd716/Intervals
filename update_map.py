        ...
        if isinstance(streams, list):
            for stream in streams:
                if isinstance(stream, dict) and stream.get("type") == "latlng":
                    ...
                    if lats and lngs:
                       # Bolt Optimization: Use a generator and islice to lazily evaluate only the retained elements,
                       # avoiding expensive round() calls on elements that will be discarded by [::4].
                       valid_pairs = ((lat, lng) for lat, lng in zip(lats, lngs) if lat is not None and lng is not None)
                       return [[round(lat, 5), round(lng, 5)] for lat, lng in itertools.islice(valid_pairs, 0, None, 4)]
    return None
